...
fetch(urlPaginaUbicacionBuscada)
 ...
 .then(data => {
  ...
  localStorage.setItem('paginaUbicacionBuscada','1') //inicializo para poder el valor y seguir con la paginacion
 })

accederUbicacionEspecifica(urlPaginaUbicacionBuscada);
...
botonSiguiente.addEventListener('click', () => {
 if (parseInt(JSON.parse(localStorage.getItem('paginaUbicacionBuscada'))) <= cantidadDePaginas) {
  contadorPaginacion++;
  localStorage.setItem('paginaUbicacionBuscada', JSON.stringify(contadorPaginacion));
  let nuevaPagina = `${urlPaginaUbicacionBuscada}&page=${contadorPaginacion}`;
  console.log(`contador siguiente: ${contadorPaginacion}`);
  paginacion(nuevaPagina);
  accederUbicacionEspecifica(nuevaPagina);
 }
...
botonAnterior.addEventListener('click', () => {
 if (parseInt(JSON.parse(localStorage.getItem('paginaUbicacionBuscada'))) > 0) {
  let paginaActual = JSON.parse(localStorage.getItem('paginaUbicacionBuscada'));
  contadorPaginacion = parseInt(paginaActual);
  contadorPaginacion--;
  localStorage.setItem('paginaUbicacionBuscada', JSON.stringify(contadorPaginacion));
  console.log(`contador anterior: ${contadorPaginacion}`);
  let paginaAnterior = `${urlPaginaUbicacionBuscada}&page=${contadorPaginacion}`;
  paginacion(paginaAnterior);
  accederUbicacionEspecifica(paginaAnterior);
 }
 else {
